...
for initialization_index=1:num_inits
    ...
    case 't_random_data_points_treat_offset_special'
        ...
        t_init(D,:) =  - normrnd(epsilon*t_mean - epsilon*t_std, t_std); % (1 x K)
    otherwise
        disp('OTHERWISE');
        error('The t wieghts init that you gace is invalid var t_initilization: %s', t_initilization);
    end
    %% c_initilization
    fprintf('1) c_initilization: %s', c_initilization);
    ...
    case 'c_kernel_mdl_as_initilization'
        disp('2) c_initilization: kernel_mdl_as_initilization')
        switch train_func_name
            case 'learn_HBF1_SGD'
                kernel_mdl = RBF(c_init,t_init,gau_precision, best_iteration_mdl.lambda);
    ...
    case 'c_normal_zeros_plus_eps'
        disp('2) c_initilization: zeros_plus_eps');
        c_init = normrnd(0,epsilon,[K,D_out]); % (D_out x K)
    case 'c_normal_ymean_ystd'
        disp('2) c_initilization: normal_ymean_ystd');
